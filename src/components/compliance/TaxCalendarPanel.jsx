...
import TaxRecordsTab from "./TaxRecordsTab";
import TaxExportsTab from "./TaxExportsTab";
...
const SUBTABS = ["deadlines", "records", "exports"];
const EMPTY_FORM = {
  title: "",
  jurisdiction: "GB",
  taxFilingType: "",
...
export default function TaxCalendarPanel({ accountId }) {
  ...
  return (
    <div className="space-y-5" data-testid="tax-calendar-panel">
      <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-900/60 dark:bg-amber-950/40">
      ...
      {subTab === "deadlines" && <DeadlinesTab accountId={accountId} />}
      {subTab === "records"   && <TaxRecordsTab accountId={accountId} />}
      {subTab === "exports"   && <TaxExportsTab accountId={accountId} />}
    </div>
  );
}
