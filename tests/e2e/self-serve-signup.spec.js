...
test("shows the self-serve signup sandbox option", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Create your Tenaqo account" })).toBeVisible();
  ...
  await expect(page.getByText("Marks this account as a safe test environment and loads sample data for demos and testing.")).toBeVisible();
