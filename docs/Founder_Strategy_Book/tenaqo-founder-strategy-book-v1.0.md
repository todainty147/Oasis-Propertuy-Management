...
- **Decision.** When resolving which account to activate on login, root operators and non-root multi-account owners share the same *mechanism* (prefer a stored account from localStorage, fall back to the first account in the list) but validate the stored ID against **different account domains**. Root validates against **all accounts in the system** (populated via `rootListAccounts`). Non-root validates against **only their own memberships** (populated via `queryMemberships` filtered by `user_id`). These two branches must not be collapsed into a single code path, even though the selection expression is currently identical on both sides — the privilege boundary lives in how `accs` is scoped upstream, and the branch makes that boundary visible.
- **Date.** 29 June 2026. **Applies:** Principles #1, #7.
- **Rationale.** Surfaced while fixing E-138 (deterministic account selection). A naive refactor proposed merging the root/non-root ternary into a single expression because both branches evaluated to `validStored ? stored : accs[0]`. The refactor was functionally correct — `accs` was already scoped per role, so `validStored` validated against the right domain implicitly. But deleting the branch removed the only visible marker that these are two different privilege levels resolving against two different account sets. The distinction looks like redundancy to any future developer or refactoring agent that sees identical code on both sides of a ternary. D-08's lesson applies: a boundary that's invisible to the person maintaining the code is a boundary waiting to be deleted.
- **The invariant this rests on (load-bearing).** Non-root `accs` must **never** contain accounts the user is not a member of. Root `accs` may contain all accounts. `my_manageable_account_ids()` enforces the same split at the RLS layer — a backstop, not a substitute for correct selection. The contract test `"validates non-root stored account against own memberships, not all accounts (E-138 deny)"` in `mediumSecurityContracts.test.js` guards this; that test must not be removed.
- **Status.** Active.
- **Revisit when.** The account-resolution logic is refactored, a new user role is introduced that crosses the root/non-root boundary, or the upstream `accs`-scoping changes. If the two branches are made genuinely different (e.g., separate `validStored` computations per role), this decision can be relaxed — the code would then defend itself without needing the branch-shape marker.
...
| v1.3 | 2026-06-29 | Added D-09 (root/non-root account-resolution domains are distinct). Surfaced by E-138 when a naive refactor proposed merging the privilege boundary. Records why identical-looking branches are not redundant — the validation domains differ even when the mechanism is the same. |
